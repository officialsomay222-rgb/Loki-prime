...
## 2025-05-27 - [Overly Permissive CORS and Missing Rate Limiter Proxy Trust]
**Vulnerability:** The Express backend used a loosely implemented CORS setup that defaulted to allowing all origins when missing, and `express-rate-limit` failed to track client IPs accurately because `app.set('trust proxy', 1)` was missing behind reverse proxies.
**Learning:** For CORS, using dynamic origin functions is safer but you must explicitly allow requests missing an `Origin` header (like mobile apps and server-to-server) to avoid breaking legitimate traffic. For rate limiters on managed hosts (Vercel, AWS), missing `trust proxy` tracks the proxy's IP, meaning one abusive user can rate-limit the entire application.
**Prevention:** Always implement explicit whitelists for CORS with proper handling for missing origins. Always set `app.set('trust proxy', 1)` when deploying Express rate limiters behind a reverse proxy.
